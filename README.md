...
두 폼은 대시보드 상단 버튼을 누르면 **페이지 이동 없이 팝업으로** 열립니다(`?embed=1` iframe). 이름은 드롭다운에서 고르고, 이메일은 첫 제출 뒤 그 기기에 기억돼 다음부터 자동으로 채워집니다 — 이메일을 서버가 이름으로 찾아주지는 않습니다. 공개 페이지라 화면의 이름을 고르는 것만으로 남의 기록을 덮을 수 있게 되기 때문입니다.
